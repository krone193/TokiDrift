...
Public Class TokiDrift
  ...
  Private Sub BtnStartOrdine_Click(sender As Object, e As EventArgs) Handles BtnStartOrdine.Click
    If TxtUser.Text <> "inserisci utente" And TxtUser.Text <> "" Then
      Dim ProgIdx = 0
      Dim dgvRecap As New DataGridView With {
       .Width = 200,
       .Height = 500,
      ...
      dgvRecap.Columns.Add(New DataGridViewTextBoxColumn)
      dgvRecap.Columns.Add(New DataGridViewTextBoxColumn)
      dgvRecap.Columns(0).HeaderCell.Value = "ID Piatto"
      dgvRecap.Columns(0).Width = 65
      dgvRecap.Columns(1).HeaderCell.Value = "Quantità Piatto"
      Dim recapForm As New Form With {
        .Width = 190,
        .Height = 510,
      ...
      recapForm.Controls.Add(dgvRecap)
      ...
      For i As Integer = 0 To ID_PiattiTot.Length - 1
        ...
        If SelezioneTotale(i) > 0 Then
          dgvRecap.Rows.Add()
          dgvRecap.Rows.Item(ProgIdx).Cells(0).Value = ID_PiattiTot(i)
          dgvRecap.Rows.Item(ProgIdx).Cells(1).Value = SelezioneTotale(i)
          ProgIdx += 1
        End If
      Next
